...
const addResultsByThread = (results, profile, include) => {
  ...
  addResult(results, `jank-${include}-bad`, durations.filter(duration => duration > 2500).length);
  addResult(results, `jank-${include}-99th`, durations.length ? quantile(durations, 0.99) : 0);
};
